...
```js
assert.match(__helpers.removeWhiteSpace(getVariance.toString()), /variance=array\.reduce\(\(acc,el\)=>\{constdifference=el-mean;constsquared=difference\*\*2;returnacc\+squared;\},0\)\/array\.length;?/);
```
